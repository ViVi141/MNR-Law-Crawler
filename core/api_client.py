...
class APIClient:
        ...
        for retry in range(max_retries):
            try:
                ...
                if retry > 0:
                    ...
                    print(f"  参数: channelid={params.get('channelid')}, page={params.get('page')}, searchword={params.get('searchword')[:50] if params.get('searchword') else ''}")
        ...
        for retry in range(max_retries):
            try:
                ...
                if content_div:
                    ...
                    if content_div.get('class') and 'content' in content_div.get('class'):
                        # 移除搜索框
                    ...
                    # 移除页面操作元素（打印、分享等）
                    for element in content_div.find_all(['div', 'span', 'p', 'a', 'button']):
                        text = element.get_text(strip=True)
                        # 移除包含页面操作关键词的短文本元素
                        if len(text) < 20 and any(keyword in text for keyword in [
                            '打印', '分享', '字号', '关闭', '下载', '仅内容打印', 
                            '【字号', '【打印', '【关闭', '【下载', '大', '中', '小'
                        ]):
                            element.decompose()
                    ...
                    for element in content_div.find_all(['div', 'span', 'p']):
                        ...
                        if not text or len(text) <= 2:
                            # 检查是否包含有用的内容
                            if not element.find_all(['img', 'a']):
                                element.decompose()
                    
                    # 优先提取 Custom_UnionStyle 中的内容（这是真正的正文）
                    custom_style = content_div.find('div', class_='Custom_UnionStyle')
                    if custom_style:
                        # 移除Custom_UnionStyle中的标题行（因为已经在YAML front matter中）
                        # 查找第一个段落，如果是标题格式，移除
                        first_p = custom_style.find('p')
                        if first_p:
                            first_p_text = first_p.get_text(strip=True)
                            # 如果第一个段落是标题（包含标题文本且格式特殊），移除
                            if len(first_p_text) > 10 and ('align="center"' in str(first_p) or 'font-size' in str(first_p)):
                                first_p.decompose()
    ...
    def _extract_metadata(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        if dtl_middle:
            ...
            if mid_1 and mid_2:
                labels_1 = [span.get_text(strip=True).replace('\xa0', '').replace('\u00a0', '') 
                           for span in mid_1.find_all('span')]
                values_1 = [span.get_text(strip=True).replace('\xa0', '').replace('\u00a0', '') 
                            for span in mid_2.find_all('span')]
                # 移除HTML注释标记
                labels_1 = [text.replace('<!--', '').replace('-->', '') for text in labels_1 if text]
                values_1 = [text.replace('<!--', '').replace('-->', '') for text in values_1 if text]
                
                # 匹配第一组的标签和值
                for i, label in enumerate(labels_1):
                    if i < len(values_1):
                        value = values_1[i]
                        label_clean = label.replace('\xa0', '').replace('\u00a0', '').replace(' ', '')
                        
                        # 提取各种元信息
                        if '发文字号' in label_clean or '文号' in label_clean:
                            if 'doc_number' not in metadata or not metadata.get('doc_number'):
                                metadata['doc_number'] = value
                        elif '发布机构' in label_clean or ('机构' in label_clean and '发布' in label_clean):
                            if 'level' not in metadata or not metadata.get('level'):
                                metadata['level'] = value
                        elif '业务类型' in label_clean or '分类' in label_clean:
                            if 'category' not in metadata or not metadata.get('category'):
                                metadata['category'] = value
            
            # 第二组：mid-3（标签）和mid-4（值）
            if mid_3 and mid_4:
                labels_3 = [span.get_text(strip=True).replace('\xa0', '').replace('\u00a0', '') 
                           for span in mid_3.find_all('span')]
                values_3 = [span.get_text(strip=True).replace('\xa0', '').replace('\u00a0', '') 
                            for span in mid_4.find_all('span')]
                # 移除HTML注释标记
                labels_3 = [text.replace('<!--', '').replace('-->', '') for text in labels_3 if text]
                values_3 = [text.replace('<!--', '').replace('-->', '') for text in values_3 if text]
                ...
                for i, label in enumerate(labels_3):
                    if i < len(values_3):
                        ...
                        if ('成文时间' in label_clean or '生成日期' in label_clean or '发布日期' in label_clean) and \
                           '效力' not in label_clean and '级别' not in label_clean:
                            # 验证值是否是日期格式（包含年月日或长度>=8）
                            if any(keyword in value for keyword in ['年', '月', '日']) or len(value) >= 8:
                                if 'pub_date' not in metadata or not metadata.get('pub_date'):
                                    metadata['pub_date'] = value
                        elif '效力级别' in label_clean or ('级别' in label_clean and '效力' in label_clean):
                            if 'validity' not in metadata or not metadata.get('validity'):
                                metadata['validity'] = value
                        elif '生效日期' in label_clean or '实施日期' in label_clean:
                            if 'effective_date' not in metadata or not metadata.get('effective_date'):
                                metadata['effective_date'] = value
                        elif '时效状态' in label_clean or ('状态' in label_clean and '时效' in label_clean):
                            if 'validity' not in metadata or not metadata.get('validity'):
                                metadata['validity'] = value
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    if len(cells) >= 2:
                        ...
                        # 提取各种元信息
                        # 发布日期：必须是日期格式，不能是效力级别
                        if ('成文时间' in label or '生成日期' in label or '发布日期' in label) and \
                           '效力' not in label and '级别' not in label:
                            # 验证值是否是日期格式（包含年月日或长度>=8）
                            if any(keyword in value for keyword in ['年', '月', '日']) or len(value) >= 8:
                                if 'pub_date' not in metadata or not metadata['pub_date']:
                                    metadata['pub_date'] = value
                            ...
                            if 'effective_date' not in metadata or not metadata['effective_date']:
                                metadata['effective_date'] = value
    ...
    def _clean_content(self, content: str) -> str:
        ...
        exact_match_keywords = [
            '【字号：大中小】', '【打印】', '【仅内容打印】', '【关闭】', '【下载】',
            '打印', '分享到', '字号', '关闭', '下载', '仅内容打印',
            '分享', '收藏', '返回顶部', '大', '中', '小'
        ]
        ...
        metadata_labels = ['名称', '文号', '发布机构', '业务类型', '废止记录', 
        ...
        while i < len(lines):
            ...
            is_metadata_label = line in metadata_labels or any(label in line for label in ['来', '一一', '源'])
            is_metadata_value = False
            
            # 检查是否是元信息值（通常是日期、机构名、文号等格式）
            if any(keyword in line for keyword in ['年', '月', '日', '部门', '规范性文件', '现行有效', '废止']):
            ...
            if is_metadata_label or is_metadata_value:
                ...
                if consecutive_metadata_lines > 5:
                    ...
                    # 跳过这个元信息块
                    while i < len(lines) and (lines[i].strip() in metadata_labels or 
                                             any(label in lines[i].strip() for label in metadata_labels) or
                                             any(keyword in lines[i].strip() for keyword in ['年', '月', '日', '部门', '规范性文件'])):
                        i += 1
                    in_metadata_block = False
                    consecutive_metadata_lines = 0
                    continue
            ...
            if len(line) < 10:
                if line in exact_match_keywords:
                    ...
                    continue
                # 检查是否是只包含这些关键词的短行
                if any(line == kw or (len(kw) < 10 and (line.startswith(kw) or line.endswith(kw))) 
            ...
            # 跳过只包含特殊字符的极短行
            if len(line) <= 2 and all(c in ' \t\n\r' for c in line):
                i += 1
                continue
            
            # 跳过只包含单个常见页面元素词的行（且行很短）
            single_word_elements = ['标题', '文号', '机构', '正文', '全部', '名称']
            if len(line) <= 3 and line in single_word_elements:
                i += 1
                continue
            ...
            # 跳过空的【】标签
            if line == '【' or line == '】' or line.startswith('【') and line.endswith('】') and len(line) <= 5:
                i += 1
                continue
            ...
            if line == '来' or line == '一一' or line == '源':
                # 检查是否是"来源"被拆分
                if i < len(lines) - 2:
                    if (line == '来' and lines[i+1].strip() == '一一' and lines[i+2].strip() == '源'):
                        # 跳过这三行
                        i += 3
                        continue
        ...
        for line in cleaned_lines:
            ...
            # 检查是否是元信息标签或值
            is_metadata = (line in metadata_labels or 
                          (len(line) <= 3 and line in metadata_labels) or
                          (len(line) < 20 and any(keyword in line for keyword in ['年', '月', '日']) and 
                           any(keyword in line for keyword in ['部门', '规范性文件', '现行有效', '废止'])))
            
            # 检查是否是真正的正文内容（长行且不是元信息）
            is_real_content = len(line) > 30 and not is_metadata
            ...
            if skip_initial_metadata:
                if is_metadata:
                    ...
                    # 不确定的行，如果已经有元信息，继续跳过；否则保留
                    if initial_metadata_count > 3:
                        continue
                    else:
                        result.append(line)
            else:
        ...
        # 移除末尾的页面元素
        while final_result and any(keyword in final_result[-1] for keyword in ['【', '】', '字号', '打印', '关闭', '下载']):
            final_result.pop()
            # 如果移除后末尾是空行，也移除
            while final_result and not final_result[-1].strip():
                final_result.pop()
